...
a <- try
a <- ifelse(try((1:2) %*% matrix(0,4,4)),1,2)
...
umau.normal.single <- function(x, cutoffs, sigma=1, alpha=.05, mu.lo=x, mu.hi=x+2, tol=1E-8) {
    mu.upper <- mu.lower <- NA
    try(mu.upper <- sigma * umau.normal.unit.var.upper.single(x/sigma, cutoffs/sigma, alpha,
                                                              mu.lo/sigma, mu.hi/sigma, tol))
    try(mu.lower <- -sigma * umau.normal.unit.var.upper.single(-x/sigma, negate.cutoffs(cutoffs)/sigma, alpha,
...
#debug(c2.single)
umau.normal(10.01, t(c(10,Inf)), tol=1E-2)
...
x.vals1side <- -7-x.vals
CIs1side <- umau.normal(x.vals1side, t(c(3,Inf)))

quants <- optim(par = c(.1,2),
                fn = function(x) { (exp(-x[1])-exp(-x[2]) - .95)^2 + (x[1]*exp(-x[1]) - x[2]*exp(-x[2]))^2 })$par

pdf("umpuUnivar.pdf",height=4,width=4)
par(mar=c(4.1,4.1,3.1,1.1))
matplot(x.vals1side[1:ncol(CIs1side)],t(CIs1side),type="l",xlim=c(0,15),ylim=c(-5,10),lty=1,col=1,
        xlab="Observed Y", ylab=expression(mu),
        main="Selective Confidence Interval")
#curve(3-quants[1]/(x-3),3.001,3.5,col="red",add=T)
#curve(3-quants[2]/(x-3),3.001,3.5,col="red",add=T)
abline(h=0,lty=3)
abline(v=3,lty=3)
abline(v=0,lty=3)
#abline(0,1,lty=3)
abline(1.96,1,lty=2)
abline(-1.96,1,lty=2)
legend("bottomright",bg="white",legend=c("Selective CI", "Nominal CI"),lty=c(1,2),bty="n")
dev.off()

exp(-quants[1])
log(.025)
quants[1]*exp(-quants[1])
quants[2]*exp(-quants[2])
curve(x*exp(-x),0,3)
